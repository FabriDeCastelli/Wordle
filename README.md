...
Laboratorio di Reti, Progetto di Fine Corso A.A. 2022/23
...
<div align="center">
  <img src="https://github.com/FabriDeCastelli/Wordle/assets/105369650/68510586-c62c-407f-b840-9d7287646f0a" style="width: 75%;" />
</div>
...
<div align="center">
  <img src="https://github.com/FabriDeCastelli/Wordle/assets/105369650/5479b787-30c6-4728-aa04-38435ef602ad" " style="width: 30%;" />
</div>
...
* login(username,password):logindiunutentegiàregistratoperaccederealservizio. Il server risponde con un codice che può indicare l’avvenuto login, oppure, se l’utente ha già effettuato la login o la password è errata, restituisce un messaggio d’errore;
* logout(username): effettua il logout dell’utente dal servizio;
* playWORDLE(): richiesta d'iniziare il gioco indovinando l’ultima parola estratta dal
server. Il server controlla se l’utente ha già partecipato al gioco per quella parola, nel qual caso invia un messaggio di errore, altrimenti il server invia un messaggio in cui indica al client che può iniziare l’invio delle Guessed Word;
* sendWord():inviodapartedelclientdiunaGuessedWordalserver.Ilserverrisponde indicando se la parola è presente nel vocabolario e, in questo caso, fornendo gli indizi. Se la parola non è presente, viene inviato un codice particolare, però il tentativo non viene contato tra i 12 tentativi consentiti;
* sendMeStatistics(): richiesta delle statistiche dell’utente aggiornata dopo l’ultimo gioco
* share() richiesta di condividere i risultati del gioco su un gruppo sociale. Come descritto nella sezione 3, il gruppo sociale verrà implementato come un gruppo di multicast;
...
* Fase di registrazione: implementata instaurando una connessione TCP con il server;
...
essere letti automaticamente da appositi file di configurazione testuali da consegnare assieme al resto del codice (due file separati per client e server). Non è consentito leggere i parametri in modo "interattivo" (ovvero facendo in modo che sia il programma a chiederli dopo essere stato avviato);
...
argomenti da passare al codice, sintassi dei comandi per eseguire le varie operazioni...). Questa sezione deve essere un manuale d'istruzioni semplice e chiaro per gli utilizzatori del sistema;
